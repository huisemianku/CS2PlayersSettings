...
namespace CS2PlayersSettings.WebApi.Controllers
{
    ...
    public class AuthController : Controller
    {
        ...
        public AuthController(UserWebApiBLL userWebApiBLL, IConfiguration configuration, ILogger<AuthController> logger)
        {
        ...
        public async Task<IActionResult> UserLogin([FromBody] LoginDto model)
        {
            ...
            try
            {
                ...
                if (user != null)
                {
                    Console.WriteLine($"UserLogin: user.UserId = {user.UserId}");
                    var token = GenerateJwtToken(user);
                    // 设置 HttpOnly Cookie
                    ...
                    return Ok(new { success = true, UserId = user.UserId, Message = "Login successful." });
                }
